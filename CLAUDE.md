...
- **Root site** (`index.html`, `css/styles.css`, `js/main.js`) — a static, dependency-free HTML/CSS/vanilla-JS marketing page ("BlockCraft Guide"). No build step: open `index.html` directly or serve the directory statically.
...
- **`projects/`** — independent sub-projects, each with its own README and conventions. Don't assume conventions from one apply to another. Each carries its own `CLAUDE.md`, loaded when you work under that directory:
  - **`prompt-engineering-mentor/`** — a repo-based prompt-engineering course, not code.
  - **`snapsite-website/`** — a static marketing site plus a small live Express backend in `server/`. The one sub-project with an automated test suite — see below.
